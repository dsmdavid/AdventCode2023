...
def find_next_position(current_position, mapping):
    ...
    for element in mapping:
        if (current_position >= element[1]) and (current_position < element[1] + element[2]):
...
for mapping in MAPPINGS:
    ...
    nm = []
    for element in mapping:
        temp = (element[1], element[1]+element[2]-1, element[0], element[0]+element[2]-1)
        nm.append(temp)
    ...
    for item in nm:
        if current[0] == item[0] or current[1] == item[1]:
            ...
            # target as in source)
            ext.append((current[0],item[0]-1, current[0],item[0]-1,))
            current = (item[1]+1, abs_max)
    nm.extend(ext)
    nm = sorted(nm)
